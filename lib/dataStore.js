...
const DEMO_SHOWCASE_ASSIGNMENTS = [
  { username: 'Animate', avatarUrl: '/demo-avatars/radio-operator.png', squadTitle: 'X-Ray', slotName: 'PLT leader' },
  { username: 'scorboutiko', avatarUrl: '/demo-avatars/squad-leader.png', squadTitle: 'Alpha', slotName: 'SQL' },
  { username: 'Dacie', avatarUrl: '/demo-avatars/combat-medic.png', squadTitle: 'Alpha', slotName: 'Medic' },
  { username: 'Scorpion', avatarUrl: '/demo-avatars/squad-leader.png', squadTitle: 'Bravo', slotName: 'SQL' },
  { username: 'Chayros', avatarUrl: '/demo-avatars/combat-medic.png', squadTitle: 'Bravo', slotName: 'Medic' },
  { username: 'Mikael', avatarUrl: '/demo-avatars/radio-operator.png', squadTitle: 'Bravo', slotName: 'Engineer/EOD' },
  { username: 'Sherlock', avatarUrl: '/demo-avatars/squad-leader.png', squadTitle: 'Charlie', slotName: 'SQL' },
  { username: 'Hemuli', avatarUrl: '/demo-avatars/combat-medic.png', squadTitle: 'Charlie', slotName: 'Medic' },
  { username: 'Divide', avatarUrl: '/demo-avatars/radio-operator.png', squadTitle: 'Charlie', slotName: 'Grenadier' }
];
...
async function ensureSchema() {
  ...
  try {
    ...
    await conn.query(`CREATE TABLE IF NOT EXISTS application_settings (
      id TINYINT PRIMARY KEY DEFAULT 1,
      enable_form_mode TINYINT(1) NOT NULL DEFAULT 1,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
    ) ENGINE=InnoDB;`);
    await conn.query('INSERT IGNORE INTO application_settings (id, enable_form_mode) VALUES (1, 1)');
...
function buildDemoTemplateData(templateId) {
  ...
  const squadIdMap = new Map();
  const squads = DEMO_DATA.template.squads.map((squad) => {
    const id = nextId++;
    squadIdMap.set(String(squad.id ?? squad.title), id);
    return {
      id,
      sourceId: squad.id ?? squad.title,
      title: squad.title,
      lrChannel: squad.lrChannel ?? 1,
      srChannel: squad.srChannel,
      marker: squad.marker,
      markerIconUrl: squad.markerIconUrl || squad.markerIcon || `/markers/${squad.marker}.svg`,
      active: squad.active !== false,
      slots: squad.slots.map((slot) => ({
        id: nextId++,
        name: slot.name,
        role: slot.role,
        allowedRoles: slot.allowedRoles || [slot.allowedRole || slot.role],
        notes: slot.notes || '',
        assignedUserId: null
      }))
    };
  ...
  return {
    ...
    squads,
    layoutNodes: (DEMO_DATA.template.layoutNodes || []).map((node) => ({
      ...node,
      squadId: squadIdMap.get(String(node.squadId)),
      parentId: node.parentId == null ? null : (squadIdMap.get(String(node.parentId)) ?? null)
    })).filter((node) => node.squadId != null),
    flowEdges: (DEMO_DATA.template.flowEdges || []).map((edge) => ({
      ...edge,
      sourceId: squadIdMap.get(String(edge.sourceId)),
      targetId: squadIdMap.get(String(edge.targetId))
    })).filter((edge) => edge.sourceId != null && edge.targetId != null)
  };
}

function saveTemplateAsDemoSeed(templateRow, templateData) {
  DEMO_DATA.template = {
    name: templateRow.name,
    description: templateRow.description || 'Saved from the template builder',
    squads: templateData.squads,
    layoutNodes: templateData.layoutNodes,
    flowEdges: templateData.flowEdges || []
  };
  writeFileSync(new URL('../data/demo-data.json', import.meta.url), `${JSON.stringify(DEMO_DATA, null, 2)}\n`, 'utf8');
}

function copyTemplateCanvasToOperation(templateData, operationSquads = []) {
  if (!Array.isArray(templateData.layoutNodes) || templateData.layoutNodes.length === 0) {
    const error = new Error('The demo template has no saved canvas layout. Save the template before copying it to the demo operation.');
    error.code = 'DEMO_LAYOUT_NOT_SAVED';
    throw error;
  }
  const operationIdByTemplateId = new Map(operationSquads
    .filter((squad) => squad.originalSquadId != null)
    .map((squad) => [String(squad.originalSquadId), squad.id]));
  const layoutNodes = templateData.layoutNodes.map((node) => ({
    ...node,
    squadId: operationIdByTemplateId.get(String(node.squadId)),
    parentId: node.parentId == null ? null : (operationIdByTemplateId.get(String(node.parentId)) ?? null)
  })).filter((node) => node.squadId != null);
  const flowEdges = (templateData.flowEdges || []).map((edge) => ({
    ...edge,
    sourceId: operationIdByTemplateId.get(String(edge.sourceId)),
    targetId: operationIdByTemplateId.get(String(edge.targetId))
  })).filter((edge) => edge.sourceId != null && edge.targetId != null);
  if (layoutNodes.length !== operationSquads.length) {
    const error = new Error('The saved template canvas does not match its squads. Save the template again before copying it to demo.');
    error.code = 'DEMO_LAYOUT_SQUAD_MISMATCH';
    throw error;
  }
  return { layoutNodes, flowEdges };
}
...
async function readData() {
  ...
  data.campaigns = campaigns.map((c) => ({
    ...objectFromJson(c.data),
    id: c.id,
    name: c.name,
    image: c.image || '',
    modlistPlayer: c.modlist_player || '',
    modlistServer: c.modlist_server || '',
    defaultTemplateId: c.default_template_id,
    missionmakerUserId: c.owner_id
  }));
...
const DEMO_CATEGORIES = ['base', 'players', 'roles', 'template', 'operation'];

function normalizeDemoCategories(categories) {
  if (!Array.isArray(categories) || categories.length === 0) return [...DEMO_CATEGORIES];
  return [...new Set(categories.filter((category) => DEMO_CATEGORIES.includes(category)))];
}

async function seedDemo(categories) {
  const selected = new Set(normalizeDemoCategories(categories));
  ...
  for (const [username, rank, status, discordId, lastPromotion, lastTraining, eligibleForTraining, roleIndexes] of DEMO_DATA.players) {
    ...
    if (!selected.has('players')) {
      if (existing[0]) demoUserIds.set(username, existing[0].id);
      continue;
    }
    ...
    demoUserIds.set(username, userId);
    const showcaseProfile = DEMO_SHOWCASE_ASSIGNMENTS.find((assignment) => assignment.username === username);
    await db.query(
      ...
      [userId, username, null, showcaseProfile?.avatarUrl || null, JSON.stringify({ discordId, lastPromotion, lastTraining, eligibleForTraining })]
    );
  }

  const withShowcaseAssignments = (squads) => squads.map((squad) => ({
    ...squad,
    slots: (squad.slots || []).map((slot) => {
      const assignment = DEMO_SHOWCASE_ASSIGNMENTS.find((item) => item.squadTitle === squad.title && item.slotName === slot.name);
      return { ...slot, assignedUserId: assignment ? (demoUserIds.get(assignment.username) || null) : null };
    })
  }));

  if (selected.has('roles')) for (const [index, name] of DEMO_DATA.roleNames.entries()) {
  ...
  if (!selected.has('template')) {
    if (templateRows.length > 0) {
      ...
      demoTemplateData = {
        ...objectFromJson(templateRows[0].data),
        squads: await getTemplateSquads(demoTemplateId)
      };
    }
    ...
    const seedSquads = demoTemplateData.squads || demoTemplateData.sections || [];
    const persistedSquads = await replaceTemplateSquads(demoTemplateId, seedSquads);
    const persistedIdBySeedId = new Map(seedSquads.map((squad, index) => [String(squad.id), persistedSquads[index]?.id]));
    demoTemplateData.layoutNodes = (demoTemplateData.layoutNodes || []).map((node) => ({
      ...node,
      squadId: persistedIdBySeedId.get(String(node.squadId)),
      parentId: node.parentId == null ? null : (persistedIdBySeedId.get(String(node.parentId)) ?? null)
    })).filter((node) => node.squadId != null);
    demoTemplateData.flowEdges = (demoTemplateData.flowEdges || []).map((edge) => ({
      ...edge,
      sourceId: persistedIdBySeedId.get(String(edge.sourceId)),
      targetId: persistedIdBySeedId.get(String(edge.targetId))
    })).filter((edge) => edge.sourceId != null && edge.targetId != null);
    demoTemplateData.squads = persistedSquads;
    const { squads: ignoredSquads, ...templateSettings } = demoTemplateData;
    await db.query('UPDATE templates SET data = ? WHERE id = ?', [JSON.stringify(templateSettings), demoTemplateId]);
  } else {
    demoTemplateId = templateRows[0].id;
    // Demo import deliberately applies the template that was explicitly exported
    // by "Save to demo", including its canvas and hierarchy.
    demoTemplateData = buildDemoTemplateData(demoTemplateId);
    const persistedSquads = await replaceTemplateSquads(demoTemplateId, demoTemplateData.squads);
    const persistedIdBySeedId = new Map(demoTemplateData.squads.map((squad, index) => [String(squad.id), persistedSquads[index]?.id]));
    demoTemplateData.layoutNodes = (demoTemplateData.layoutNodes || []).map((node) => ({
      ...node,
      squadId: persistedIdBySeedId.get(String(node.squadId)),
      parentId: node.parentId == null ? null : (persistedIdBySeedId.get(String(node.parentId)) ?? null)
    ...
    demoTemplateData.squads = persistedSquads;
    const { squads: ignoredSquads, ...templateSettings } = demoTemplateData;
    await db.query('UPDATE templates SET name = ?, description = ?, data = ? WHERE id = ?', [
      DEMO_DATA.template.name,
      DEMO_DATA.template.description,
      JSON.stringify(templateSettings),
      demoTemplateId
    ]);
  }
  if (selected.has('operation') && (!demoTemplateData || !Array.isArray(demoTemplateData.layoutNodes) || demoTemplateData.layoutNodes.length === 0)) {
    const error = new Error('The demo template has no saved canvas layout. Save the template before copying it to the demo operation.');
    error.code = 'DEMO_LAYOUT_NOT_SAVED';
    throw error;
  }
  ...
  if (demoOps.length === 0) {
    ...
    const squads = withShowcaseAssignments(demoTemplateData.squads || demoTemplateData.sections || []);
    const demoPayload = {
      name: 'Thursday Operation Demo',
      templateId: demoTemplateId,
      date: demoDate,
      ...DEMO_DATA.operationSettings,
      squads: squads.map((squad) => ({ ...squad, originalSquadId: squad.id, slots: squad.slots.map((slot) => ({ ...slot, originalSlotId: slot.id })) }))
    };
    ...
    demoOpId = opResult.insertId;
  } else {
    demoOpId = demoOps[0].id;
    let existingPayload = {};
    try {
  ...
  const operationSquads = await getOperationSquads(demoOpId);
  const operationCanvas = copyTemplateCanvasToOperation(demoTemplateData, operationSquads);
  const [storedOpRows] = await db.query('SELECT payload FROM ops WHERE id = ?', [demoOpId]);
  const storedOpPayload = objectFromJson(storedOpRows[0]?.payload);
  await db.query('UPDATE ops SET payload = ? WHERE id = ?', [JSON.stringify({ ...storedOpPayload, ...operationCanvas }), demoOpId]);
  }
  ...
  return {
    ...
    defaultOpSettings: {
      ...DEMO_DATA.operationSettings,
      ...(demoTemplateId ? { templateId: demoTemplateId } : {})
    }
  };
}

async function getDemoStatus() {
  const playerNames = DEMO_DATA.players.map((player) => player[0]);
  const roleNames = DEMO_DATA.roleNames;
  const placeholders = (values) => values.map(() => '?').join(',');
  const rankConditions = DEFAULT_RANKS.map(() => '(id = ? AND name = ?)').join(' OR ');
  const rankValues = DEFAULT_RANKS.flatMap((rank) => [rank.id, rank.name]);
  const squadConditions = DEFAULT_SQUAD_TYPES.map(() => '(id = ? AND name = ?)').join(' OR ');
  const squadValues = DEFAULT_SQUAD_TYPES.flatMap((squad) => [squad.id, squad.name]);
  const [[rankRows], [squadRows], [playerRows], [roleRows], [templateRows], [operationRows]] = await Promise.all([
    db.query(`SELECT COUNT(*) AS count FROM ranks WHERE ${rankConditions}`, rankValues),
    db.query(`SELECT COUNT(*) AS count FROM squad_types WHERE ${squadConditions}`, squadValues),
    db.query(`SELECT COUNT(*) AS count FROM users WHERE username IN (${placeholders(playerNames)}) AND password_hash = 'demo-login-disabled'`, playerNames),
    db.query(`SELECT COUNT(*) AS count FROM roles WHERE name IN (${placeholders(roleNames)}) AND JSON_UNQUOTE(JSON_EXTRACT(metadata, '$.source')) = ?`, [...roleNames, 'TFO main docs / Playerlist']),
    db.query('SELECT COUNT(*) AS count FROM templates WHERE owner_id IS NULL AND name IN (?, ?)', [DEMO_DATA.template.name, 'Demo Template']),
    db.query('SELECT COUNT(*) AS count FROM ops WHERE owner_id IS NULL AND title IN (?, ?)', ['Thursday Operation Demo', 'Demo Operation'])
  ]);
  return {
    base: { count: Number(rankRows[0]?.count || 0) + Number(squadRows[0]?.count || 0), expected: DEFAULT_RANKS.length + DEFAULT_SQUAD_TYPES.length },
    players: { count: Number(playerRows[0]?.count || 0), expected: playerNames.length },
    roles: { count: Number(roleRows[0]?.count || 0), expected: roleNames.length },
    template: { count: Number(templateRows[0]?.count || 0), expected: 1 },
    operation: { count: Number(operationRows[0]?.count || 0), expected: 1 }
  };
}

async function deleteDemo(categories) {
  const selected = new Set(normalizeDemoCategories(categories));
  const conn = await db.getConnection();
  const deleted = {};
  try {
    await conn.beginTransaction();
    if (selected.has('operation')) {
      const [result] = await conn.query('DELETE FROM ops WHERE owner_id IS NULL AND title IN (?, ?)', ['Thursday Operation Demo', 'Demo Operation']);
      deleted.operation = result.affectedRows;
    }
    if (selected.has('template')) {
      const [templates] = await conn.query('SELECT id FROM templates WHERE owner_id IS NULL AND name IN (?, ?)', [DEMO_DATA.template.name, 'Demo Template']);
      const ids = templates.map((row) => row.id);
      if (ids.length) {
        await conn.query(`UPDATE ops SET template_id = NULL WHERE template_id IN (${ids.map(() => '?').join(',')})`, ids);
        const [result] = await conn.query(`DELETE FROM templates WHERE id IN (${ids.map(() => '?').join(',')})`, ids);
        deleted.template = result.affectedRows;
      } else deleted.template = 0;
    }
    if (selected.has('roles')) {
      const names = DEMO_DATA.roleNames;
      const [result] = await conn.query(`DELETE FROM roles WHERE name IN (${names.map(() => '?').join(',')}) AND JSON_UNQUOTE(JSON_EXTRACT(metadata, '$.source')) = ?`, [...names, 'TFO main docs / Playerlist']);
      deleted.roles = result.affectedRows;
    }
    if (selected.has('players')) {
      const names = DEMO_DATA.players.map((player) => player[0]);
      const [users] = await conn.query(`SELECT id FROM users WHERE username IN (${names.map(() => '?').join(',')}) AND password_hash = 'demo-login-disabled'`, names);
      const ids = users.map((row) => row.id);
      if (ids.length) {
        await conn.query(`UPDATE operation_slots SET assigned_user_id = NULL WHERE assigned_user_id IN (${ids.map(() => '?').join(',')})`, ids);
        const [result] = await conn.query(`DELETE FROM users WHERE id IN (${ids.map(() => '?').join(',')})`, ids);
        deleted.players = result.affectedRows;
      } else deleted.players = 0;
    }
    if (selected.has('base')) {
      const rankConditions = DEFAULT_RANKS.map(() => '(id = ? AND name = ?)').join(' OR ');
      const rankValues = DEFAULT_RANKS.flatMap((rank) => [rank.id, rank.name]);
      const squadConditions = DEFAULT_SQUAD_TYPES.map(() => '(id = ? AND name = ?)').join(' OR ');
      const squadValues = DEFAULT_SQUAD_TYPES.flatMap((squad) => [squad.id, squad.name]);
      const [rankResult] = await conn.query(`DELETE FROM ranks WHERE ${rankConditions}`, rankValues);
      const [squadResult] = await conn.query(`DELETE FROM squad_types WHERE ${squadConditions}`, squadValues);
      deleted.base = rankResult.affectedRows + squadResult.affectedRows;
    }
    await conn.commit();
    return { categories: [...selected], deleted };
  } catch (error) {
    await conn.rollback();
    throw error;
  } finally {
...
async function copyTemplateToDemo(templateId) {
  const [templateRows] = await db.query('SELECT id, name, description, data FROM templates WHERE id = ?', [templateId]);
  if (!templateRows[0]) {
    const error = new Error('Template not found');
    error.code = 'DEMO_TEMPLATE_NOT_FOUND';
    throw error;
  }
  const templateData = {
    ...objectFromJson(templateRows[0].data),
    squads: await getTemplateSquads(templateId)
  };
  if (!Array.isArray(templateData.layoutNodes) || templateData.layoutNodes.length === 0) {
    const error = new Error('Save the template canvas before copying it to demo.');
    error.code = 'DEMO_LAYOUT_NOT_SAVED';
    throw error;
  }

  const [rows] = await db.query("SELECT id, payload FROM ops WHERE title = 'Thursday Operation Demo' ORDER BY id DESC LIMIT 1");
  const existingPayload = objectFromJson(rows[0]?.payload);
  const demoDate = existingPayload.date || new Date().toISOString().slice(0, 10);
  const scheduledAt = `${demoDate} ${DEMO_DATA.operationSettings.time}:00`;
  let operationId = rows[0]?.id;
  if (!operationId) {
    const [result] = await db.query(
      'INSERT INTO ops (template_id, title, scheduled_at, timezone, recurrence, payload, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [templateId, 'Thursday Operation Demo', scheduledAt, 'Europe/Amsterdam', JSON.stringify(null), JSON.stringify({}), 'scheduled']
    );
    operationId = result.insertId;
  }

  const operationInput = templateData.squads.map((squad) => ({
    ...squad,
    originalSquadId: squad.id,
    slots: (squad.slots || []).map((slot) => ({ ...slot, originalSlotId: slot.id, assignedUserId: null }))
  }));
  await replaceOperationSquads(operationId, operationInput);
  const operationSquads = await getOperationSquads(operationId);
  const canvas = copyTemplateCanvasToOperation(templateData, operationSquads);
  const payload = {
    ...existingPayload,
    name: 'Thursday Operation Demo',
    templateId,
  ...
  await db.query(
    'UPDATE ops SET template_id = ?, title = ?, scheduled_at = ?, timezone = ?, payload = ? WHERE id = ?',
    [templateId, 'Thursday Operation Demo', scheduledAt, 'Europe/Amsterdam', JSON.stringify(payload), operationId]
  );
  saveTemplateAsDemoSeed(templateRows[0], templateData);
  return { operationId, templateId, layoutNodes: canvas.layoutNodes.length, flowEdges: canvas.flowEdges.length };
}

export { readData, writeData, ensureInitialized, ensureSchema, resetDatabase, seedEssential, seedDemo, getDemoStatus, deleteDemo, copyTemplateToDemo };
