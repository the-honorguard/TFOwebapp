...
async function ensureInitialized() {
  ...
  if (ranks[0].c === 0) {
    for (const r of DEFAULT_RANKS) {
      await db.query('INSERT INTO ranks (id, name, abbreviation, order_index, icon) VALUES (?, ?, ?, ?, ?)', [r.id, r.name, r.short || r.abbreviation || null, r.order, r.icon || null]);
    }
  }

  // If there are no ops/templates yet, insert a tiny demo template and demo op
  // so public visitors see something on the Overview page by default.
