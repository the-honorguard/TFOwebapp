...
async function ensureSchema() {
  ...
  try {
    ...
    try { await conn.query('ALTER TABLE template_squads ADD CONSTRAINT template_squads_parent_fk FOREIGN KEY (parent_squad_id) REFERENCES template_squads(id) ON DELETE SET NULL'); } catch (error) { if (!['ER_FK_DUP_NAME', 'ER_DUP_KEYNAME'].includes(error?.code)) throw error; }
